    ...
    <div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        ...
        <div className="bg-muted rounded-lg p-4">
          ...
          <p className="text-xs text-muted-foreground mt-1">Contact dept admin to modify</p>
        </div>
      </div>

      {/* Documents */}
      <h2 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider mb-3">Documents (read-only)</h2>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {(docsQuery.data?.documents as Record<string, unknown>[] ?? []).map((doc) => (
              <tr key={String(doc._id)} className="border-b border-border">
                <td className="px-4 py-3">{doc.original_name as string ?? doc.file_name as string}</td>
                <td className="px-4 py-3">
                  <span className={`text-xs ${doc.ingestion_status === 'completed' ? 'text-green-700 dark:text-green-400' : doc.ingestion_status === 'failed' ? 'text-destructive' : 'text-yellow-700 dark:text-yellow-400'}`}>
            ...
            {!docsQuery.data?.documents?.length && (
              <tr><td colSpan={3} className="px-4 py-6 text-center text-muted-foreground">No documents uploaded.</td></tr>
            )}
          </tbody>
        </table>
      </div>
