...
export default function CollegeAdminDashboard() {
  ...
  if (dashQuery.isLoading) return <p className="text-muted-foreground text-sm">Loading dashboard…</p>;
  ...
  return (
    <div>
      ...
      {/* Departments Table */}
      <h2 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider mb-3">Departments</h2>
          ...
          <tbody>
            {depts
              ...
              .map((dept) => {
                ...
                return (
                  <tr key={String(d._id)} className="border-b border-border hover:bg-accent/40">
                    <td className="px-4 py-3 font-medium">{d.name as string}</td>
                    <td className="px-4 py-3 text-foreground">{admin?.name as string ?? <span className="text-yellow-700 dark:text-yellow-400 text-xs">No admin</span>}</td>
                    <td className="px-4 py-3 text-foreground">{d.document_count as number}</td>
                    <td className="px-4 py-3">
                      {admin
                        ? <span className="text-green-700 dark:text-green-400 text-xs">Active</span>
                        : <span className="text-yellow-700 dark:text-yellow-400 text-xs">Vacant</span>}
                    </td>
                    <td className="px-4 py-3">
                      <button
      ...
      {/* Faculty List */}
      <h2 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider mb-3">Faculty (Dept Admins)</h2>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {deptAdmins.map((admin) => {
              ...
              return (
                <tr key={String(a._id)} className="border-b border-border hover:bg-accent/40">
                  <td className="px-4 py-3">
                    <p>{a.name as string}</p>
                    <p className="text-muted-foreground text-xs">{a.email as string}</p>
                  </td>
                  <td className="px-4 py-3 text-foreground">{dept?.name as string ?? '—'}</td>
                  <td className="px-4 py-3">
                    <span className={`text-xs ${a.status === 'active' ? 'text-green-700 dark:text-green-400' : a.status === 'invited' ? 'text-yellow-700 dark:text-yellow-400' : 'text-destructive'}`}>
                      {a.status as string}
                    </span>
                  </td>
