  ...
  return (
    <div>
      ...
      {!isLoading && !isError && filtered.length === 0 && (
        <p className="text-muted-foreground text-sm">No {tab} students found.</p>
      )}
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtered.map((s, i) => (
                <tr
                  key={s._id}
                  className={`border-t border-border ${i % 2 === 0 ? 'bg-card' : 'bg-muted'} hover:bg-gray-750 transition-colors`}
                >
                  <td className="px-4 py-2.5 font-medium">
                    {s.name}
                    {s.using_generic_fallback && (
                      <span className="ml-2 text-xs text-amber-700 dark:text-amber-400">fallback</span>
                    )}
                  </td>
                  <td className="px-4 py-2.5 text-muted-foreground">{s.email}</td>
                  <td className="px-4 py-2.5 text-muted-foreground">{s.roll_number ?? '—'}</td>
                  {(depts && depts.length > 1) && (
                    <td className="px-4 py-2.5 text-muted-foreground">
                      {deptMap[s.dept_id] ?? '—'}
                    </td>
                  )}
                  <td className="px-4 py-2.5 text-muted-foreground">{yearLabel} {s.semester}</td>
                      ...
                      {tab === 'pending' ? (
                        <>
                          <button
                            ...
                            className="text-xs text-green-700 dark:text-green-400 hover:text-green-700 dark:text-green-300 px-2 py-1 border border-green-700 rounded"
                          >
                            Approve
                          </button>
                          <button
                      ...
                      {confirmDeleteId === s._id ? (
                        <span className="flex items-center gap-1">
                          ...
                          <button
                            ...
                            className="text-xs text-destructive hover:text-red-700 dark:hover:text-red-300 px-2 py-1 border border-red-300 dark:border-red-700 rounded"
                          >
                            Yes
                          </button>
                          <button
                        ...
                        <button
                          ...
                          className="text-xs text-destructive hover:text-destructive px-2 py-1 border border-red-300 dark:border-red-800 rounded"
                        >
                          Delete
                        </button>
                      )}
