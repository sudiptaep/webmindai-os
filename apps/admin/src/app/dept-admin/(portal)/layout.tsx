...
export default function DeptAdminLayout({ children }: { children: React.ReactNode }) {
  ...
  useIdleLogout(handleLogout, mounted && !!token && user?.role === 'dept_admin');
          ...
          {NAV.map(({ href, label }) => {
            ...
            return (
              <Link key={href} href={href}
                className={`block px-3 py-2 rounded text-sm transition-colors ${active ? 'bg-indigo-600 text-white' : 'text-foreground hover:bg-muted'}`}>
                {label}
              </Link>
            );
          })}
