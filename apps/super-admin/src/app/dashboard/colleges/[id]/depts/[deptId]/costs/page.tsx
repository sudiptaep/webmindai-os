    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <Link href={`/dashboard/colleges/${collegeId}/depts/${deptId}/policy`}
            className="bg-primary text-primary-foreground hover:bg-primary/90 rounded-lg px-3 py-1.5 text-sm text-white transition-colors">
            Edit Policy
          </Link>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-card border border-border rounded-xl p-5">
          <h2 className="text-sm font-semibold text-foreground mb-4">Cost by Action Type</h2>
              ...
              <div className="mt-3 space-y-1">
                {actionBreakdown.sort((a, b) => b.total_cost_usd - a.total_cost_usd).map((a) => (
                  <div key={a.action_type} className="flex items-center justify-between text-xs">
                    <span className="text-muted-foreground">{ACTION_LABELS[a.action_type] ?? a.action_type}</span>
                    <div className="flex gap-4 text-muted-foreground">
                      <span>{fmtK(a.count)} calls</span>
                      <span className="font-mono text-foreground">{fmt$(a.total_cost_usd)}</span>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="bg-card border border-border rounded-xl p-5">
                ...
                return (
                  <div key={s.service}>
                    <div className="flex items-center justify-between text-xs mb-1">
                      <span className="text-foreground capitalize">{s.service.replace('_', ' ')}</span>
                      <div className="flex gap-3 text-muted-foreground">
                        <span>{fmtK(s.total_tokens)} tokens</span>
                        <span className="font-mono text-foreground">{fmt$(s.total_cost_usd)}</span>
                      </div>
                    </div>
          ...
          {perQueryAnalysis && (
            <div className="mt-5 pt-4 border-t border-border">
              <p className="text-xs font-semibold text-muted-foreground mb-2">Per-Query Analysis</p>
      ...
      {topStudents.length > 0 && (
        <div className="bg-card border border-border rounded-xl p-5">
          <h2 className="text-sm font-semibold text-foreground mb-4">Top Students by Token Usage (masked)</h2>
                ...
                {topStudents.map((s, i) => {
                  ...
                  return (
                    <tr key={s.student_id} className="hover:bg-muted/40">
                      ...
                      <td className="py-2 text-muted-foreground font-mono text-xs">{masked}</td>
                      ...
                      <td className="py-2 text-right text-muted-foreground text-xs">{fmtK(s.chat_message_count)}</td>
                    </tr>
                  );
                })}
              </tbody>
