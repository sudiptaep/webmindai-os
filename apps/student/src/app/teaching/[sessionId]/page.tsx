  ...
  return (
    <div className="flex flex-col h-[calc(100vh-0px)] max-w-2xl mx-auto">
      <div className="px-4 sm:px-6 pt-4 pb-2 border-b border-border">
        <button onClick={() => router.push('/teaching')} className="text-xs text-muted-foreground hover:text-foreground mb-2">
          ← Teach me
        </button>
      ...
      {isComplete ? (
        <div className="px-4 sm:px-6 py-4 border-t border-border flex gap-3">
          ...
          <button
            ...
            className="flex-1 bg-muted hover:bg-accent text-foreground text-sm rounded-lg py-2.5"
          >
            Review queue
          </button>
        </div>
        ...
        <div className="border-t border-border px-4 sm:px-6 py-3 space-y-2">
          <div className="flex flex-wrap gap-1.5">
            {CONTROLS.map((c) => (
              <button
                ...
                disabled={busy}
                className="text-xs bg-card hover:bg-muted border border-border text-muted-foreground rounded-full px-3 py-1 disabled:opacity-40"
              >
                {c.label}
              </button>
            ))}
          ...
          <div className="flex gap-2">
            <textarea
              ...
              rows={1}
              className="flex-1 bg-card border border-border rounded-lg px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-teal-700 resize-none"
            />
            <button
              onClick={send}
          ...
          {(respond.isError || control.isError) && (
            <p className="text-xs text-destructive">{(respond.error ?? control.error)?.message}</p>
          )}
        </div>
      )}
    </div>
        ...
        <div key={name} className="flex items-center gap-1 shrink-0">
          <div
            className={`w-2 h-2 rounded-full ${
              i < currentPhase ? 'bg-teal-600' : i === currentPhase ? 'bg-teal-400 ring-2 ring-teal-900' : 'bg-accent'
            }`}
          />
          {i < PHASE_NAMES.length - 1 && <div className={`w-3 h-px ${i < currentPhase ? 'bg-teal-700' : 'bg-muted'}`} />}
        </div>
      ))}
  ...
  if (turn.role === 'student') {
    return (
      <div className="flex justify-end">
        <div className="max-w-[80%] bg-teal-100 dark:bg-teal-800/40 border border-teal-300 dark:border-teal-800/60 text-teal-900 dark:text-teal-50 text-sm rounded-2xl rounded-br-sm px-4 py-2 whitespace-pre-wrap">
          {turn.content}
        </div>
      </div>
    );
    ...
    <div className="flex justify-start">
      <div className="max-w-[85%] space-y-2">
        {turn.isBacktrackNotice && (
          <p className="text-xs text-amber-700 dark:text-amber-400 flex items-center gap-1">⏸ Pausing to cover a gap first</p>
        )}
        <div
          className={`text-sm rounded-2xl rounded-bl-sm px-4 py-3 whitespace-pre-wrap ${
            turn.isComplete
              ? 'bg-card border border-teal-300 dark:border-teal-900 text-foreground'
