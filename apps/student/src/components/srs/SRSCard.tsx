    ...
    <div className="bg-card border border-border/60 rounded-xl overflow-hidden">
      ...
      <div className="px-4 py-2.5 border-b border-border/60 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] text-muted-foreground capitalize">{card.bloom_level}</span>
        </div>
        <button
          onClick={onSuspend}
      ...
      <div className="px-5 py-5">
        ...
        <p className="text-foreground text-sm leading-relaxed">{card.question_text}</p>
        ...
        {isMCQ && !revealed && (
          <div className="mt-4 space-y-2">
            {card.options.map((opt, i) => (
              <div
                key={i}
                className="flex items-start gap-2.5 px-3 py-2.5 rounded-lg border border-border/40 text-sm text-foreground"
              >
      ...
      {!revealed ? (
        <div className="px-5 pb-5">
          <button
            onClick={onReveal}
            className="w-full py-2.5 rounded-lg border border-teal-700/50 text-teal-700 dark:text-teal-400 hover:bg-teal-700/10 text-sm font-medium transition-colors"
          >
            Show Answer
          </button>
        </div>
        ...
        <div className="px-5 pb-5 border-t border-border/60 pt-4 space-y-3">
          {/* Answer */}
          <div>
            <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-1.5">Answer</p>
            <p className="text-teal-700 dark:text-teal-300 text-sm leading-relaxed font-medium">{card.correct_answer}</p>
          </div>
                ...
                return (
                  <div
                    ...
                    className={`flex items-start gap-2.5 px-3 py-2 rounded-lg border text-xs ${
                      isCorrect
                        ? 'border-teal-700/60 bg-teal-100 dark:bg-teal-900/20 text-teal-700 dark:text-teal-300'
