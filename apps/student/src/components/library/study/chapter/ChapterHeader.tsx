...
export function ChapterHeader({ chapter }: Props) {
  return (
    <div className="px-5 py-3 border-b border-border shrink-0 bg-card/50">
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="text-right shrink-0">
          <p className="text-xs text-muted-foreground">Pg {chapter.start_page}–{chapter.end_page}</p>
      ...
      {chapter.pyq_count > 0 && (
        <div className="flex items-center gap-2 mt-2 flex-wrap">
          ...
          {chapter.pyq_years.map(y => (
            <span
              key={y}
              className="text-xs bg-amber-100 dark:bg-amber-900/30 text-amber-500 border border-amber-800/50 px-1.5 py-0.5 rounded"
            >
              {y}
            </span>
          ))}
