...
export const analyticsRouter = router({
  ...
  queryVolume: deptAdminProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      // dept_admin always sees only their one dept
      const deptFilter = [ctx.user.dept_id];
      ...
      const result = await QueryLog.aggregate([
        ...
        { $limit: 50 },
        {
          $project: {
            ...
            count: 1, first_seen: 1, last_seen: 1, query_ids: 1,
          },
        },
      ]);

      return result as Array<{
        dept_id: string; query_text: string; count: number;
        first_seen: Date; last_seen: Date; query_ids: string[];
      }>;
    }),

  // Acknowledge unanswered query
  acknowledgeQuery: deptAdminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (ctx.user.dept_id !== log.dept_id) {
        throw new TRPCError({ code: "FORBIDDEN", message: "Dept scope not permitted" });
      }
